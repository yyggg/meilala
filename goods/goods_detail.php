<?php
	...
	$sql = "SELECT gid,name,market_price,price,ispay,prepay,gimg,doctor_hpt,parameter,contens FROM m_goods WHERE gid = '$gid'";
	$sth = $db->prepare($sql);
	$sth->execute();
	...
	//赞，收藏，评价
	$sql = "SELECT count(1) as count FROM m_house_zan WHERE `pid` = '$gid' AND `type` = '0' AND `acttype` = '0'";
	$sth = $db->prepare($sql);
	...
	$sql = "SELECT count(1) as count FROM m_house_zan WHERE pid = '$gid' AND type = '0' AND acttype = '1'";
	$sth = $db->prepare($sql);
	$sth->execute();
	$zan = $sth->fetchColumn();
...
<div class="content mb10">
    ...
    <div class="detail" id="detail1">
        ...
        <div class="detail_ct pb10 mb10">
            ...
            -->
            <div class="goods_price_li"><div class="left">原价:</div><div class="right" style="text-decoration: line-through;">&yen;<?php echo number_format($data['market_price']);?></div></div>
            <div class="goods_price_li"><div class="left">美啦啦优惠价：</div><div class="right">&yen;<?php echo number_format($data['price']);?></div></div>
            <div class="goods_price_li" style="border-bottom:0rem;"><div class="left">预约金：</div><div class="right"><i>&yen;<?php echo number_format($data['prepay']);?></i></div></div>
            <div class="callus">
                <button onclick="window.location.href='goods_apply.php?gid=<?php echo $gid; ?>'">我要预订</button>
            </div>
            <div class="operate">
