<?php
	...
	$sql = "SELECT count(1) as count FROM m_comment WHERE `pid` = '$gid' AND `type` = '0' AND `mid` = '$mid'";
	$sth = $db->prepare($sql);
	$sth->execute();
	...
	if($_POST){
		...
		else
		{
			...
			{
				$addJf = $user['jifen']+5;
				$sql = "INSERT INTO m_jifen_logs SET mid = '$mid', gtime = '$time', point = '5', income='1', remark='评论项目', count = '$addJf'";
				$sth = $db->prepare($sql);
				$sth->execute();
				if($db->lastInsertId())
				{
			...
			//同步项目表评论数
			$sql = "UPDATE m_goods SET pl = pl+1 WHERE gid = '$gid' LIMIT 1";
			$sth = $db->prepare($sql);
			$sth->execute();
		}
		die(json_encode($res));
