<?php
	...
	$sql = "SELECT gid,name,market_price,price,gimg,doctor_hpt,parameter,contens FROM m_goods WHERE gid = '$gid'";
	$sth = $db->prepare($sql);
	$sth->execute();
...
<!--header-->
<header class="header">
	<div class="header_back"><a href="/goods/goods.php?typeid=<?php echo $typeid;?>"></a></div>
    <h2>项目详情</h2>
</header>
<!--header end-->
<!--content-->
...
window.onscroll = function(){ 
	...
	if(t<=h){
		$('.detail_top').css({"position":"relative"});
	}
    else { 
		$('.detail_top').css({"position":"fixed","width":"100%","top":"0px","max-width":"640px","margin-left":"auto","margin-right":"auto"});
    }
} 
