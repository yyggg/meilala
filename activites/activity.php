<?php
	...
	if(isset($_GET['ajax-goods']))
	{
		...
		$sql = "SELECT aid,name,aimg,enroll,quota,stime,etime,flag FROM m_activites $where ORDER BY flag DESC , aid DESC LIMIT $start, $pageSize";
		...
		'loader': function (data){
			...
			if(data.length)
			{
				$.each(data, function(k, v){
					...
					if(v.flag.indexOf('b')>=0){
						var html = '<div class="item_li act_li act_new_li" id="atv_'+v.aid+'" style="opacity:0;-moz-opacity: 0;filter: alpha(opacity=0);">' +
								'<a href="activity_detail.php?aid='+v.aid+'">'+
								'<div class="pic" style=""><img src="<?php echo IMG_PATH;?>'+v.aimg+'" alt="" /></div>'+
								'<div class="tt">'+v.name+'</div>';
							if(cdTime) html += '<div class="time">剩<i>00</i>天<i>23</i>时<i>59</i>分<i>59</i>秒</div>';
							html +=	'</a></div>';
					}else{
						var html = '<div class="item_li act_li" id="atv_'+v.aid+'" style="opacity:0;-moz-opacity: 0;filter: alpha(opacity=0);">' +
								...
								'<div class="title">'+v.flag+v.name +'</div>' +
