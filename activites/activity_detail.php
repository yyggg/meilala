<?php
	...
	$sql = "SELECT aid,name,aimg,stime,etime,quota,enroll,contens,zan,sc,pl FROM m_activites WHERE aid = :aid";
	$sth = $db->prepare($sql);
	...
	//是否已报名
	$sql = "SELECT eid FROM m_atv_enroll WHERE aid = '$aid' AND mid='$user[mid]' LIMIT 1";
	$sth = $db->prepare($sql);
	$sth->execute();
	$isApply = $sth->fetch(PDO::FETCH_ASSOC);

	//获取订单号
	$sql = "SELECT id ,order_no,stat  FROM m_order WHERE pid =  $aid AND type = '1' AND mid='$user[mid]' order by stat desc ";
	$sth = $db->prepare($sql);
	$sth->execute();
	$my_order = $sth->fetch();
	//print_r($my_order);
...
<?php
include_once '../inc/activity_hhr.php';
?>
</body>
</html>
