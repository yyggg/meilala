<?php 
	...
	$sql = "SELECT count(1) as count FROM m_message WHERE mid = '{$user["mid"]}' AND `read` = 0 AND type=1 AND url !=''";
	
	$sth = $db->prepare($sql);
	$sth->execute();
	$msgCount = $sth->fetchColumn();
...
<div class="content space">
	<ul>
        ...
        <li onclick="window.location.href='/member/my_order.php'" class="space_li">
            <i class="icon icon11"></i><h3>我的订单</h3><i class="go"></i>
        </li>        
		<li onclick="window.location.href='/member/my_house.php'" class="space_li">
        	<i class="icon icon2"></i><h3>我的收藏</h3><i class="go"></i>
        </li>
		<li onclick="window.location.href='/member/msg_list.php'" class="space_li">
        	<i class="icon icon3"></i><h3>我的消息<?php if($msgCount):;?><span class="space_num"><?php echo $msgCount;?></span><?php endif;?></h3><i class="go"></i>
        </li>        
    ...
    <ul>
		...
		<li onclick="window.location.href='/member/my_comment.php'" class="space_li">
