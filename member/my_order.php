<?php
	...
	if(isset($_GET['ajax_order']))
	{
		...
		foreach($data as $k=>$v)
		{
			...
			$data[$k]['name'] =	get_activites_item( 'name',$v['pid'],$v['type']);
			$data[$k]['stat'] = $v['stat']==1?'<em class="stat1">支付成功</em>':'<em class="stat2">待支付</em>';
		}
		die(json_encode($data));  //转换为json数据输出
	}

	function get_activites_item($item='*',$pid , $type)
	{
		global $db;
		if($type==1){
			$sql = "SELECT $item FROM m_activites WHERE aid = $pid ";
		}
		else{
			$sql = "SELECT $item FROM m_goods WHERE gid = $pid ";
		}		
		$sth = $db->prepare($sql);
		$sth->execute();
		...
		'loader': function (data){
			if(data.length)
			{
				$.each(data, function(k, v){
					if(v.stat=='<em class="stat2">待支付</em>'){
						var url='../plus/go_pay.php?order_no='+v.order_no+'&type='+v.type;
						var stat='<em class="stat2">待支付</em>';
					}
					else{
						if(v.type==1){
							var url='../activites/activity_detail.php?aid='+v.pid;
						}
						else{
							var url='../goods/goods_detail.php?gid='+v.pid;
						}
						var stat='<em class="stat1">支付成功</em>'
					}
					var type = v.type==1?'活动支付订单':'项目支付订单';
					var html = '<li class="order_li mt10" onclick="window.location.href=\''+url+'\'">'+
