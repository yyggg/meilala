(function($) {
	...
	$.fn.welivebar_update = function(sScroll) {
		//alert(111);
		$('#overview').scrollTop($('#overview')[0].scrollHeight+40);
		$('.viewport').scrollTop($('.viewport')[0].scrollHeight+40);
	...
	function Scrollbar($container, options) {
		...
		function wheel(event) {
			if (contentRatio < 1) {
				...
				$thumb.css(posiLabel, contentPosition / (trackRatio == 0 ? 1 : trackRatio));
				$overview.css(posiLabel, -contentPosition);
				//document.getElementById('overview').scrollTop = document.getElementById('overview').scrollHeight;
