...
//if(!SYSDIR){var SYSDIR=window.location.host;alert(SYSDIR);}
//alert(SYSDIR);
...
function getLocalTime() {
    ...
    return addZeros(date.getFullYear(),4) +'-'+ addZeros(date.getMonth()+1,2) +'-'+ addZeros(date.getDate(),2) +'&nbsp'+ addZeros(date.getHours(), 2) + ':' + addZeros(date.getMinutes(), 2) + ':' + addZeros(date.getSeconds(), 2)
}
...
function format_output(data) {
	...
	data = data.replace(/\[img:(\S*):\]/g, '<div class="upload_img_box"  ><img src="' + SYSDIR + '/plus/tar_img.php?p=60&i=http://'+window.location.host+ SYSDIR +'$1" class="upload_img" onclick="look_big_img(this.src); " style="height:220px;" ></div>').replace(/\\/g, '');
    return data
}
...
function welive_link() {
    //alert(111);
	//alert(headimgurl);
	//guest.openid=guest.openid?guest.openid:'guest'+guest.gid;
	//guest.gid	=guest.gid?guest.gid:'0';
    //$.post('wx_user.php?x=9',{openid: guest.openid,gid:guest.gid}); 
	//ajax('wx_user.php?x=9',{openid: guest.openid,gid:guest.gid});    
	...
	case 6:
		...
		case 8:
			...
			$.each(data.re, function(i, rec) {
				...
				*/
				if (rec.t == 1) {
					recs += '<div class="time">' + rec.d + '</div><div class="msg l"><img src="images/var_img.png" class="var_img fl"/><b></b><div class="b a_msg"><div class="i ">' + format_output(rec.m) + '</div></div></div><div style="clear:both" ></div>'
				} else {
					recs += '<div class="time">' + rec.d + '</div><div class="msg r"><img src="../images/touxiang.png" class="var_img fr"/><b></b><div class="b"><div class="i ">' + format_output(rec.m) + '</div></div></div><div style="clear:both" ></div>'
				}				
			});
			if (recs != '') {
...
*/
	switch (type) {
        case 1:
            d = '<div class="time">' + getLocalTime() + '</div><div class="msg l"><img src="images/var_img.png" class="var_img fl"/><b></b><div class="b a_msg"><div class="i ">' + format_output(d) + '</div></div></div><div style="clear:both" ></div>';
            break;
        case 2:
            d = '<div class="time">' + getLocalTime() + '</div><div class="msg r"><img src="../images/touxiang.png" class="var_img fr"/><b></b><div class="b"><div class="i ">' + format_output(d) + '</div></div></div><div style="clear:both" ></div>';
            break;
        case 3:
            d = '<div class="msg s"><div class="b"><div class="ico"></div><div class="i">' + d + '</div></div></div>';
            break;
...
function welive_verify() {
    ...
    welive.ws.send('x=6&a=8&gid=' + guest.gid + '&fn=' + guest.fn + '&aid=' + guest.aid + '&l=' + guest.lang + '&k=' + SYSKEY + '&c=' + SYSCODE + '&fr=' + guest.fromurl + '&ag=' + guest.agent + '&i=' + 'meilala')
}
...
function welive_comment() {
    ...
    historier.css("padding-bottom", 0).html('').append('<div class="comment"><form id="comment_form" onsubmit="return false;"><input type="hidden" name="vid" value="' + vid + '"><input type="hidden" name="key" value="' + SYSKEY + '"><input type="hidden" name="code" value="' + SYSCODE + '"><li><b>' + langs.yourname + ':</b><input name="fullname" type="text"><i>*</i></li><li><b>' + langs.email + ':</b><input name="email" type="text" ><i>*</i></li><li><b>' + langs.phone + ':</b><input name="phone" type="text" ></li><li><b>' + langs.content + ':</b><textarea name="content" ></textarea><i>*</i></li><li style="display:none"><b></b><img src="' + SYSDIR + 'welive1618.php?ajax=1&act=get&vid=' + vid + '" onclick="ChangeCaptcha(this);" title="' + langs.newcaptcha + '"> = <input name="vvc" type="text" class="vvc" value="2"><i>*</i></li></form></div>')
}
...
function welive_init() {
	...
	//welive.ic = welive_op.find("#welive_copyright>a").attr("href");
    welive.ic = 'meilala.net';
    welive_link();
